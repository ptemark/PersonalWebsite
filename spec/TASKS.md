...
| 63 | 2026-03-03 | Fill in package.json metadata | package.json | Set name "petermark-dev", description, author "Peter Mark <peter.sw.mark@gmail.com>", license "MIT", cleared empty keywords array; build passes |
| 64 | 2026-03-03 | js/app.js null safety | js/app.js | Guard toggleBtn in applyTheme() with inline if(toggleBtn) checks; wrap toggleBtn event listener in if(toggleBtn) block; guard nav scroll listener in if(nav) block; closeMenu() early-returns if(!nav || !hamburgerBtn); wrap hamburger+outside-click+Escape+resize listeners in if(hamburgerBtn && nav) block; wrap backToTopBtn scroll+click listeners in if(backToTopBtn) block; build passes |
| 65 | 2026-03-03 | Hobbies chips accessibility | index.html, css/style.css | Changed hobbies__chips container from div to ul; changed hobbies__chip from span to li; added list-style:none/margin:0/padding:0 to .hobbies__chips to reset ul browser defaults; transition was already wrapped in prefers-reduced-motion guard (task 59); build passes |
| 66 | 2026-03-03 | CSS custom properties for accent tints | css/style.css | Added --color-accent-tint and --color-accent-tint-border to :root (dark values) and [data-theme="light"] (light values); replaced all four hardcoded rgba() values in .projects__feature-tag with the new custom properties; removed now-redundant [data-theme="light"] .projects__feature-tag override block; build passes |
